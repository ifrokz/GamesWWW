function Player(){
    ...
    function disparar(){
        ...
        if(ms-that.lastShot > that.shotMinTime){
            ...
            that.shots.push(new Shot(that.posX,that.posY,that.shotSpeed,that.shotSpeed,that.shotSize,that.shotDmg,that.shotDir,that.speedX,that.speedY,that.slowDownRate));
    ...
    this.draw = function(){
      ...
      //  console.log(this.spriteNum+""+this.spriteDirection)
        ...
        if(roomData.type != 0){  // GAP EN X
            if(this.posX+170/2 > game.windowWidth/2 && this.posX+170/2+game.windowWidth/2 <= roomData.width){
                //console.log(roomData.type);
                dibujarX = game.windowWidth/2-170/2;
                tempGapX = dibujarX-this.posX;
                game.setGapX(tempGapX);
            }else if(this.posX+170/2+game.windowWidth/2 >= roomData.width ){
                dibujarX = this.posX+roomData.gapX;
            }
        }
        
        if(roomData.type > 1){
            if(this.posY+this.height/2 > game.windowHeight/2 && this.posY+this.height/2+game.windowHeight/2 <= roomData.height){
                dibujarY = game.windowHeight/2-this.height/2;
                tempGapY = dibujarY-this.posY;
                game.setGapY(tempGapY);
            }else if(this.posY+this.height/2+game.windowHeight/2 >= roomData.height){
                dibujarY = this.posY+roomData.gapY;
            }
        }
